...
function fetchDanhSachKhuVuc() {
    ...
    fetch(`${API_BASE_URL}/khu-vuc/chu-tro/${chuTroId}`)
        ...
        .then(data => {
            ...
            data.forEach(kv => {
                // VALUE lưu ID (1, 2, 3...), TEXT hiển thị Tên
                select.innerHTML += `<option value="${kv.id}">${kv.tenKhuVuc}</option>`;
            });
            select.addEventListener('change', reloadCurrentTab);
        })

...
function setupKiemTraTrungLap(inputId, type) {
    ...
    inputEl.addEventListener('input', function() {
        ...
        typingTimer = setTimeout(() => {
            fetch(`${API_URL_KHACH}/chu-tro/${chuTroId}/kiem-tra-danh-tinh?${type}=${val}`)
                ...
                .then(data => {
                    ...
                    // 1. TRƯỜNG HỢP: KHÁCH CŨ QUAY LẠI (TRÙNG NỘI BỘ)
                    if (state === "TRUNG_NOI_BO") {
                        ...
                        if (errorEl && errorEl.classList.contains('invalid-feedback')) {
                            // Nếu là số điện thoại đang dùng ở phòng khác
                            if (type === 'sdt' && data.canhBao) {
                                errorEl.innerHTML = `⚠️ SĐT đang dùng cho ${data.canhBao}`;
                            } else {
                                errorEl.innerHTML = `✅ Đã tìm thấy hồ sơ cũ (${autoFillDataTam.tenKhach})`;
                            }
                            errorEl.style.display = 'block';
                            
                            // Thêm nút Điền nhanh vào giao diện
                    ...
                    // 2. TRƯỜNG HỢP: TRÙNG HỆ THỐNG (Khách của chủ trọ khác)
                    else if (state === "TRUNG_HETHONG") {
                        if (data.noXau) {
                            // Khách có nợ xấu -> Báo đỏ
                            ...
                            // Khách lịch sử tốt -> Coi như mới, cho qua bình thường
                            this.classList.remove('is-invalid');
                            this.classList.add('is-valid');
                            if (errorEl && errorEl.classList.contains('invalid-feedback')) errorEl.innerText = "";
                        }
                    } 
                    // 3. TRƯỜNG HỢP: MỚI HOÀN TOÀN
                    else {
                        this.classList.remove('is-invalid');
                        this.classList.add('is-valid');
                        if (errorEl && errorEl.classList.contains('invalid-feedback')) errorEl.innerText = "";
        ...
        }, 600); // Đợi gõ xong 0.6s mới gọi API
    });
}
...
// Hàm thực thi việc tự động đổ dữ liệu khách cũ vào form
function dienNhanhThongTinCu(data, source) {
    ...
    // Xử lý ngày sinh (đổi từ YYYY-MM-DD sang DD/MM/YYYY)
    ...
    // Biến tất cả thành ô hợp lệ (màu xanh)
    ...
    // Tắt nút điền nhanh và xóa thông báo
    ...
    // Gắn ngầm ID cũ để Backend hiểu là cập nhật/gia hạn cho khách này
    document.getElementById('khachId').value = data.id;
}
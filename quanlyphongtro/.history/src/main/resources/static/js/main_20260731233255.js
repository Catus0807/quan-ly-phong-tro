...
function fetchDanhSachKhuVuc() {
    ...
    fetch(`${API_BASE_URL}/khu-vuc/chu-tro/${chuTroId}`)
        ...
        .then(data => {
            ...
            if (!select) return; 
            select.innerHTML = '-- Tất cả khu vực --'; 
            
            data.forEach(kv => {
                // VALUE BẮT BUỘC LÀ ID (kv.id) ĐỂ TRÁNH LỖI TRÙNG TÊN CHI NHÁNH
                select.innerHTML += `${kv.tenKhuVuc}`;
            });
            
            // Cập nhật lại màn hình hiện tại sau khi đã có danh sách khu vực
            reloadCurrentTab();
        })

...
function deleteRoom(id) {
    ...
    // Tính năng thêm nhanh Chi nhánh mới từ Modal
    function themKhuVucMoi() {
        const chuTroId = localStorage.getItem('chuTroId');
        if (!chuTroId) return alert("Vui lòng đăng nhập lại!");

        const tenKhuVuc = prompt("Nhập tên Chi nhánh / Khu vực mới:\n(Ví dụ: Cơ sở 1 - Cầu Giấy, Tòa A, Nhà trọ số 5...)");
        
        // Nếu người dùng ấn Hủy hoặc để trống thì thoát
        if (!tenKhuVuc || tenKhuVuc.trim() === "") return;

        const data = {
            tenKhuVuc: tenKhuVuc.trim(),
            diaChi: tenKhuVuc.trim() // Tạm gán địa chỉ giống tên khu vực
        };
        ...
        .then(newKhuVuc => {
            ...
            // 1. Cập nhật lại bộ lọc tổng ở ngoài màn hình
            if (typeof fetchDanhSachKhuVuc === 'function') {
                fetchDanhSachKhuVuc(); 
            }
            
            // 2. Thêm thẻ option mới vào thẳng Modal đang mở và tự động chọn nó
            const modalKhuVucSelect = document.getElementById('roomKhuVuc');

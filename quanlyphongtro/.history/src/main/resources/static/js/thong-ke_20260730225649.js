...
function fetchThongKe() {
    ...
    if (thangInput && thangInput.value) {
        ...
        if (switchCaNam && switchCaNam.checked) {
            // NẾU BẬT "CẢ NĂM": Khóa input, đổi tiêu đề, gửi chuỗi "ALL/Năm"
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            // Cập nhật lại UI dựa trên dữ liệu trả về
...
function renderDuLieuThongKe(data) {
    // KIỂM TRA XEM ĐANG LÀ BÁO CÁO NĂM HAY THÁNG
    ...
    // Ngăn không gán chuỗi "ALL/2026" vào ô input date gây lỗi ngầm
    const thangInput = document.getElementById('inputThangThongKe');
...
function fetchWidgetCanhBao(chuTroId, chiNhanh, thangNam) {
    ...
    fetch(urlHd)
        ...
        .then(data => {
            ...
            const noList = data.filter(hd => hd.trangThai === 'CHUA_THU');
    ...
    fetch(urlKhach)
        ...
        .then(data => {
            ...
            today.setHours(0, 0, 0, 0); // Đưa về 0h để tính ngày chuẩn xác

            hetHanList = hetHanList.filter(k => {
                // Tách chuỗi YYYY-MM-DD để tạo Date theo giờ địa phương (Tránh lỗi chênh lệch UTC)
            ...
            if(hetHanList.length === 0) {
                ...
                hetHanList.slice(0, 5).forEach(k => {
                    // Tính lại chính xác số ngày dùng cùng logic giờ địa phương
                    const [y, m, d] = k.ngayKetThuc.split('-');
                    ...
                    if (soNgay < 0) {
                        ...
                        // Hợp đồng > 30 ngày nhưng lọt vào vì nằm trong năm/tháng đang xem
...
function xuatBaoCaoExcel(loai) {
    ...
    if (loai === 'nam') {
        ...
        Promise.all([
            ...
            let dsThang = hoaDons.filter(hd => hd.thangThu === thangHienThi);
            if (chiNhanhId) {
            ...
            let dsSuCoThang = suCos.filter(sc => {
                ...
                // Kiểm tra có đúng tháng đang xem không?
            ...
            // Nếu người dùng có chọn Lọc theo Chi Nhánh
            if (chiNhanhId) {
                dsSuCoThang = dsSuCoThang.filter(sc => {
                    // Nếu là sự cố trong phòng: so sánh ID khu vực
                    if (sc.phongTro && sc.phongTro.khuVuc) {
                        return String(sc.phongTro.khuVuc.id) === String(chiNhanhId);
                    }
                    // Nếu là sự cố KHU VỰC CHUNG (phong_id null): so sánh chuỗi tên chi nhánh
            ...
            if (dsSuCoThang.length === 0) {
                ...
                dsSuCoThang.forEach((sc, i) => {
                    ...
                    // (Nếu có phòng thì thêm chữ "P.", nếu là khu vực chung thì in thẳng tên vị trí)

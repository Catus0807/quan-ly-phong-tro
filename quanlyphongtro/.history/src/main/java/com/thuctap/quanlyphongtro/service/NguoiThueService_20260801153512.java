...
public class NguoiThueService {
    ...
    @Autowired
    private HoaDonRepository hoaDonRepository; // Nhớ import thư viện này ở đầu file nếu chưa có

    // Thuật toán kiểm tra và phân loại khách hàng
    public Map kiemTraDanhTinhThongMinh(Long chuTroId, String cccd, String sdt) {
        Map result = new java.util.HashMap<>();
        result.put("loaiTrung", "KHONG_TRUNG");
        
        Optional khachTimThay = Optional.empty();
        ...
        if (khachTimThay.isPresent()) {
            ...
            // Nếu khách này đã/đang ở hệ thống trọ của người khác (Xuyên hệ thống)
                ...
                if (khach.getPhongTro() != null) {
                    List noCuoc = hoaDonRepository.findByTrangThai("CHUA_THU").stream()

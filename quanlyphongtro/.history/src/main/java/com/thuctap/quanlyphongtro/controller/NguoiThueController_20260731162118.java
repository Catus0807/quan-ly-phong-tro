...
public class NguoiThueController {
    ...
    @GetMapping("/kiem-tra/cccd")
    public ResponseEntity<?> checkCccd(@RequestParam("cccd") String cccd) { 
        return ResponseEntity.ok(Map.of("exists", nguoiThueService.checkCccdExists(cccd)));
    }

    @GetMapping("/kiem-tra/sdt")
    public ResponseEntity<?> checkSdt(@RequestParam("sdt") String sdt) { 
        return ResponseEntity.ok(Map.of("exists", nguoiThueService.checkSdtExists(sdt)));
    }

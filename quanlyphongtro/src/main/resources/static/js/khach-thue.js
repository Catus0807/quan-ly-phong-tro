...
let autoFillDataTam = null; // Biến lưu tạm dữ liệu khách cũ để điền nhanh
...
function setupKiemTraTrungLap(inputId, type) {
    ...
    inputEl.addEventListener('input', function() {
        ...
        const oldBtn = inputEl.parentNode.querySelector('.btn-fill-nhanh');
        if (oldBtn) oldBtn.remove();
        
        if (this.disabled) return;
        
        if (!val || (type === 'sdt' && val.length < 10) || (type === 'cccd' && val.length < 12)) {
            this.classList.remove('is-valid', 'is-invalid', 'border-warning');
        ...
        typingTimer = setTimeout(() => {
            fetch(`${API_URL_KHACH}/chu-tro/${chuTroId}/kiem-tra-danh-tinh?${type}=${val}`)
                .then(res => res.json())
                .then(data => {
                    const state = data.loaiTrung;
                    
                    // 1. TRƯỜNG HỢP: KHÁCH CŨ QUAY LẠI HOẶC MUỐN THUÊ THÊM
                    if (state === "TRUNG_NOI_BO") {
                        autoFillDataTam = data.data;
                        
                        this.classList.remove('is-invalid');
                        this.classList.add('is-valid', 'border-success');
                        
                        if (errorEl && errorEl.classList.contains('invalid-feedback')) {
                            // ĐÃ THÊM: Hiện dòng nhắc nhở phòng đang thuê
                            let msgHtml = `✅ Đã tìm thấy hồ sơ cũ (${autoFillDataTam.tenKhach})`;
                            
                            if (data.canhBao) {
                                msgHtml += `⚠️ Nhắc nhở: Khách này hiện ĐANG THUÊ ${data.canhBao}`;
                            }
                            ...
                            btnFill.className = "btn btn-sm btn-success btn-fill-nhanh mt-2 w-100 fw-bold shadow-sm";
                            btnFill.innerHTML = ` Sử dụng lại hồ sơ này`;
                            btnFill.type = "button";
                            btnFill.onclick = () => dienNhanhThongTinCu(autoFillDataTam, type);
                            errorEl.parentNode.appendChild(btnFill);
                        }
                    } 
                    // 2. TRƯỜNG HỢP: TRÙNG HỆ THỐNG
                    else if (state === "TRUNG_HETHONG") {
                        if (data.noXau) {
                            this.classList.add('is-invalid');
                            if (errorEl && errorEl.classList.contains('invalid-feedback')) {
                                errorEl.innerHTML = `❌ Khách thuê có lịch sử nợ xấu/chưa đóng tiền trên hệ thống!`;
                                errorEl.style.display = 'block';
                            }
                        } else {
                            this.classList.remove('is-invalid');
...
function dienNhanhThongTinCu(data, source) {
    if (!data) return;
    
    document.getElementById('tenKhach').value = data.tenKhach || '';
    document.getElementById('cccd').value = data.cccd || '';
    document.getElementById('sdt').value = data.sdt || '';
    document.getElementById('ktGioiTinh').value = data.gioiTinh || '';
    document.getElementById('ktQueQuan').value = data.queQuan || '';
    ...
    ['tenKhach', 'cccd', 'sdt', 'ktGioiTinh', 'ktQueQuan', 'ktNgaySinh'].forEach(id => {
    ...
    const btn = document.querySelector('.btn-fill-nhanh');
    if (btn) btn.remove();
    
    const feedbackEl = source === 'cccd' ? document.getElementById('err-cccd') : document.getElementById('err-sdt');
    if (feedbackEl) {
        feedbackEl.innerText = "";
        feedbackEl.style.display = 'none';
    }
    
    // BƯỚC NGOẶT QUAN TRỌNG: Không gán ID cũ vào form nữa!
    // Bằng cách để trống ID, khi ấn "Lưu Hợp Đồng", Frontend sẽ ra lệnh POST tạo ra một bản ghi mới (hợp đồng thứ 2) thay vì sửa phòng cũ (PUT).
    document.getElementById('khachId').value = ''; 
}
...
function khoaOChuaHopLe(input, exactLength, errorMsg) {
    ...
    // Không báo lỗi chiều dài nếu ô đang báo lỗi nợ xấu hoặc đã bắt được khách cũ
    if (input.classList.contains('is-valid') || (feedback && feedback.innerText.includes('lịch sử'))) {
        return; 
    }
    
    if (!val) { 
        input.classList.add('is-invalid');
        if (feedback && feedback.classList.contains('invalid-feedback')) feedback.innerText = 'Không được để trống!';
    } else if (val.length !== exactLength) { 
        input.classList.add('is-invalid');
        if (feedback && feedback.classList.contains('invalid-feedback')) feedback.innerText = errorMsg;
    } else { 
...
function moChiTietNguoiGhepTuThongBao(khachId) {
    ...
    if (khach) {
        // Nếu có người ở ghép, lấy người mới nhất (nằm ở cuối mảng) để hiển thị
        if (khach.danhSachNguoiOGhep && khach.danhSachNguoiOGhep.length > 0) {
            const nguoiGhepMoi = khach.danhSachNguoiOGhep[khach.danhSachNguoiOGhep.length - 1];
            xemChiTietCaNhan(nguoiGhepMoi, true); 
        } else {
            // Nếu vì lý do nào đó không tìm thấy người ghép, mở thông tin khách chính
            xemChiTietCaNhan(khach, false);
        }
    }
}